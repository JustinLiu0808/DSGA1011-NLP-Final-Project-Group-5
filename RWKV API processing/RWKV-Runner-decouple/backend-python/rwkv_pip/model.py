...
if os.environ.get("RWKV_V7_ON") == "1":
    ...
    class RWKV_x070(MyModule):
        def __init__(self, model, strategy):
            ...
            super().__init__()
            self.eval()
            # ===== Logging buffers for analysis (hidden states & temporal dynamics) =====
            # hidden_to_log: list of per-layer hidden activations (already used in notebooks)
            self.hidden_to_log = []
            # temporal_logs[layer_id] = {"lambda_k": Tensor[T, C], "lambda_v": Tensor[T, C]}
            self.temporal_logs = {}
            # memory_logs[layer_id] = {"D_t": Tensor[T], "N_t": Tensor[T]}
            self.memory_logs = {}
            # flags: turn on only when做分析实验，默认不记录以避免额外开销
            self.log_temporal = False
        ...
        def _reset_logs(self):
            """Clear logging buffers before each forward pass."""
            self.hidden_to_log = []
        ...
        def _log_temporal(self, layer_id: int, lambda_k: torch.Tensor, lambda_v: torch.Tensor):
            """
            Store per-layer temporal decay samples for analysis.
            lambda_k, lambda_v: shape [T, C] on any device.
            """
            lk = lambda_k.detach().to("cpu")
            lv = lambda_v.detach().to("cpu")
            self.temporal_logs[layer_id] = {
                "lambda_k": lk,
                "lambda_v": lv,
            }

        def _log_memory(self, layer_id: int, D_t: torch.Tensor, N_t: torch.Tensor):
            """
            Store per-layer memory persistence proxies over time.
            D_t, N_t: shape [T] (sequence-length curves).
            """
            Dt = D_t.detach().to("cpu")
            Nt = N_t.detach().to("cpu")
            self.memory_logs[layer_id] = {
                "D_t": Dt,
                "N_t": Nt,
            }

        def forward(self, idx, state, full_output=False):
            # reset logging buffers for this forward pass
            self._reset_logs()
            if state == None:
        ...
        def forward_one(self, idx: int, state: List[torch.Tensor]):
            with torch.no_grad():
                ...
                for i in range(self.n_layer):
                    ...
                    # =============================
                    # ADD PRINT OR SAVE HERE !!!
                    # =============================
                    print(f"[forward_one] layer {i} x shape = {x.shape}, min={x.min().item():.4f}, max={x.max().item():.4f}")
                    # save hidden state
                    self.hidden_to_log.append(x.detach().cpu())
        ...
        def forward_seq(
            ...
            with torch.no_grad():
                ...
                for i in range(self.n_layer):
                    ...
                    # ADD PRINT 
                    # =============================
                    print(f"[forward_seq] layer {i} x shape = {x.shape}")
                    self.hidden_to_log.append(x.detach().cpu())
        ...
        if os.environ.get("RWKV_CUDA_ON") == "1":
            ...
            def RWKV_x070_TMix_seq(
                ...
                # w is in log-decay domain here; later used inside custom CUDA op
                w = -torch.nn.functional.softplus(-(w0 + w)) - 0.5

                # ===== Logging for analysis experiments (inference-only, optional) =====
                if self.log_temporal or self.log_memory:
                    # Effective decay factor λ^k per time step & channel.
                    # Approximate λ^k = exp(w_log) where w_log is the log-decay fed into CUDA kernel.
                    lambda_k = torch.exp(w)
                    # Use attention gate 'a' as a proxy for λ^v (value-related temporal modulation).
                    lambda_v = a

                    if self.log_temporal:
                        self._log_temporal(layer_id, lambda_k, lambda_v)

                    if self.log_memory:
                        # D_t: sequence of average decay factors over channels
                        D_t = lambda_k.mean(dim=-1)  # [T]
                        # N_t: proxy for memory "injection" magnitude from current step
                        N_t = (k * v).abs().mean(dim=-1)  # [T]
                        self._log_memory(layer_id, D_t, N_t)
            ...
            def RWKV_x070_TMix_seq(
                ...
                # ===== Logging for analysis experiments (CPU fallback) =====
                if self.log_temporal or self.log_memory:
                    # In CPU path, w is already the decay factor used in recurrence.
                    lambda_k = w
                    lambda_v = a
